...
static const u8 FONTSET[FONT_LENGTH] = { 
...
void core_init(Chip_walo *cw) {
    ...
    for (u32 font = 0; font < FONT_LENGTH; ++font) {
        cw->mem[START_FONT_ADDRESS] = FONTSET[font];
    }

    cw->pc = START_MEM_ADDRESS;
}

void core_load(Chip_walo *cw, const u8 *rom) {
    uint64_t rom_size = 0;
    u8 *rom_buffer = NULL;
    FILE *file = NULL;
    ...
    if (status > 0) {
        if (rom_size <= (MEMORY_SIZE - START_MEM_ADDRESS)) {
            for (u32 data = 0; data < rom_size; ++data) {
                cw->mem[START_MEM_ADDRESS + data] = rom_buffer[data];
            }
        } else {
