...
void draw_graphics()
{
    ...
    for (uint8_t ycor = 0; ycor < SCREEN_HEIGHT; ycor++) {
        for (uint8_t xcor = 0; xcor < SCREEN_WIDTH; xcor++) {
...
{
    uint16_t *buffer = (short *)rawBuffer;
    uint16_t length = bytes / 2;
    uint16_t sampleNR = (*(int *) userData);

    for(uint16_t data = 0; data < length; data++, sampleNR++)
    {
        double_t time = (double)sampleNR / (double)SAMPLE_RATE;
